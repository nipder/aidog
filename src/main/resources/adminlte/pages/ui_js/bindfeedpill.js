...
$(function () {
    ...
    $('.select-all').on('click', function () {
        if (this.checked) {
            $('.checkbox.feedcheck').each(function () {
                this.checked = true;
            });
        } else {
            $('.checkbox.feedcheck').each(function () {
    ...
    $("#a_bindfeedpill").click(function () {
        var feedids = $("#selectfeeds").html();
        var pill_code = hamletcode = $('#select_pill').find('option:selected').val();
        var configtime = $("#input_lay").val();

        if (feedids == "" || feedids == null) {
            alert("请先批量获取喂饲器！");
            return;
        }
        ...
        var data = {};
        data.mids = feedids.substring(0, feedids.length - 1);
        data.pill_code = pill_code;
        data.configtime = configtime;
        $.ajax({
            url: "/aidog/api/batchfeedpillbind",
            type: "POST",
            dataType : "json",
...
function choosedog() {
    ...
    var arr = []
    var spanarr = "";
    feedcheck.map(function (item) {
        if (feedcheck[item].checked) {
            arr.push(feedcheck[item].id);
            spanarr += feedcheck[item].id + "|";
        }
    })
    spanarr = spanarr.replace(/\"/g, "");
    $("#selectfeeds").html(spanarr);
    $("#doglist").modal('hide');
}
